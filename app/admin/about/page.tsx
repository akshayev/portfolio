  ...
  return (
    <PageContainer title="about">
      <section className="space-y-3">
        ...
        <AboutSectionForm initial={null} />
      </section>

      <section className="space-y-3">
        <h2 className="text-sm font-semibold uppercase tracking-wider text-slate-500">Existing Records</h2>
        {data.length === 0 ? (
          <EmptyState message="No about records found yet. Create one to get started." />
        ) : (
          <div className="space-y-5">
            {data.map((record) => (
              <AboutSectionForm key={record.id} initial={record} canDelete={canDelete} />
            ))}
          </div>
        )}
      </section>
