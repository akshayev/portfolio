  ...
  return (
    <PageContainer title="settings/visual">
      <section className="space-y-3">
        ...
        <GlobalVisualSettingsForm initial={null} />
      </section>

      <section className="space-y-3">
        <h2 className="text-sm font-semibold uppercase tracking-wider text-slate-500">Existing Records</h2>
        {data.length === 0 ? (
          <EmptyState message="No visual settings records found yet. Create one to get started." />
        ) : (
          <div className="space-y-5">
            {data.map((record) => (
              <GlobalVisualSettingsForm key={record.id} initial={record} canDelete={canDelete} />
            ))}
          </div>
        )}
      </section>
