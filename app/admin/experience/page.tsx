import { ExperienceForm } from "@/components/admin/forms/section-forms";
  ...
  return (
    <PageContainer title="experience">
      ...
      <section className="space-y-3">
        ...
        {data.length === 0 ? (
          <EmptyState message="No experience records found yet. Create one to get started." />
        ) : (
          <div className="space-y-5">
            {data.map((record) => (
              <ExperienceForm key={record.id} initial={record} canDelete={canDelete} />
            ))}
          </div>
        )}
      </section>
